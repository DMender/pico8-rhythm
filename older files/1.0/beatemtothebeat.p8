...
-- [[movement normalization
	norm = 1
	if (btn(る⬆️) or btn(る⬇️)) and (btn(る⬅️) or btn(る➡️)) then
		norm = 1
	else
		norm = 1.4
	end--]]
...
-- [[all beats
	...
	if (flr(note/rate))%2 == 0 and not btn(る🅾️) and not btn(る❎) then
		active = true
...
-- [[movement
	if adjust_hits <= 12 or btn(る🅾️) or btn(る❎) and clover_state != clover_stunned then
	else
		-- up
		if btn(る⬆️) and player.y > 66 and clover_state != clover_stunned then

			clover_state = clover_walk[anim_frame]
			player.y -= 1 * player.speed * norm
		-- down
		elseif btn(る⬇️) and player.y < 95 and clover_state != clover_stunned then
			clover_state = clover_walk[anim_frame]
			player.y += 1 * player.speed * norm
		end
		-- left
		if btn(る⬅️)  and player.x > 0 and clover_state != clover_stunned then
			clover_state = clover_walk[anim_frame]
			player.flipped = -4
			facing_flip = true
			player.x -= 1.7 * player.speed * norm
		-- right
		elseif btn(る➡️) and player.x < 112 and clover_state != clover_stunned then
			clover_state = clover_walk[anim_frame]
			player.flipped = 4
			facing_flip = false
			player.x += 1.7 * player.speed * norm
...
-- [[punch
	if btnp(る❎) and active and clover_state != clover_stunned then
		whiff_left = false
		whiff_right = false
		beat = 167
		sfx(1)
	...
	elseif btn(る❎) and clover_state != clover_stunned then
		whiff_left = true
		whiff_right = true
...
-- [[kick
	if btnp(る🅾️) and active and synco and clover_state != clover_stunned then
		whiff_left = false
		whiff_right = false
		beat = 167
		sfx(1)
	...
	elseif btn(る🅾️) and clover_state != clover_stunned then
		whiff_left = true
		whiff_right = true
		beat = 166
...
-- [[calibration
	if adjust_hits <= 12 and btnp(る❎) then
			adjust_hits += 1
	end--]]
...
-- [[red caps
	...
	for k,i in pairs(spawn) do
		...
		if distance(player.x+8,player.y,i.x,i.y) < 24  and i.x > player.x+8 then
			reserved_right = true
			right_queue += 1
			if not whiff_right and not facing_flip and (btnp(る🅾️) or btnp(る❎)) then
				i.life -= 1
				i.state = "stunned"
				whiff_right = true
				if btnp(る🅾️) then
		...
		elseif distance(player.x,player.y,i.x+8,i.y) < 24  and i.x+8 < player.x then
			reserved_left = true
			left_queue += 1
			if not whiff_left and facing_flip and (btnp(る🅾️) or btnp(る❎)) then
				i.life -= 1
				i.state = "stunned"
				whiff_left = true
				if btnp(る🅾️) then
...
-- track
	...
	spr(168,note_x+88,120)
	spr(168,note_x+96,120)
	spr(168,note_x+104,120)
	spr(168,note_x+112,120)
	spr(168,note_x+120,120)
	spr(168,note_x+128,120)
	spr(beat,0,120)
	spr(x,64,100)
	spr(z,54,100)
	else
...

